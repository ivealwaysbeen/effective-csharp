...
> * 생성자를 작성할 때 동일한 작업을 되풀이하는 경우, 기존에 작성한 코드를 그대로 복사하지 않는다. (중복 코드 최소화)
> * C++에서는 공통 수행 작업을 Private 메서드로 관리했지만, C#에서는 공용으로 사용할 수 있는 생성자를 작성하는 것이 더 장점이 많다.
>> * 공용 생성자는 변수에 대한 중복 초기화 코드를 제거해주고, 베이스 클래스의 생성자가 중복 호출되는 것을 막아준다. (코드의 양도 줄일 수 있다)
>> * Private 메서드를 사용하여 공통 수행 작업을 관리할 경우 readonly 변수를 초기화할 수 없고, 컴파일 시 모든 생성자에 초기화 구문과 베이스 클래스 생성자 코드가 생성된다.
> * 기본 매개변수 기능을 활용하면 여러 개의 생성자를 하나로 바인딩하는 효과를 볼 수 있다. 하지만 이를 사용하는 코드와의 결합도가 높아진다는 단점이 있다.
> * 기본 매개변수 기능을 사용할 때는 기본값이 합리적이어야 한다. (코드의 변화에도 사용할 수 있어야하고, 예외 상황이 발생하지 않는 값이어야 한다)
> * ### 첫 번째 인스턴스가 생성될 때 수행되는 과정
...
>> 7. 배이스 클래스 인스턴스 변수 생성자 수행
>> 8. 인스턴스 생성자 수행
> * 클래스 자체에 대하 초기화 작업은 한번마 이루어지기 때문에, 두 번째 인스턴스 생성부터는 5단계 부터 진행된다.
  
15. 불필요한 객체를 만들지 말라
> * 자주 사용되는 객체는 멤버변수로 선언하여 반복적으로 생성하지 않도록 하고, 재사용하여야 한다.
> * IDisposable 인터페이스를 구현한 타입의 객체를 멤버변수로 사용할 경우, 해당 클래스도 반드시 IDisposable을 구현해야 한다.
> * 유사한 객체를 반복적으로 사용할 때는 정적 멤버 변수를 사용하는 것이 바람직하다.
> * immutable 타입의 객체는 변경될 때마다 새로운 객체를 생성하므로, 효율적으로 관리해야 한다. ex) string -> stringBuilder

16. 생성자 내에서는 절대로 가상 함수를 호출하지 말라
> * C#에 정의에 따르면, 생성자가 호출되는 순간 해당 객체는 이미 초기화가 완료된 것으로 간주된다. 
> * 때문에 만약 베이스 클래스의 생성자에서 가상 함수를 호출하고 있고, 멤버 변수를 참조하여 함수가 작동한다면, 실질적인 초기화 여부에 따라 결과가 다르게 나올 수 있다.
> * C++의 경우 각 클래스의 생성자가 실행될 때의 객체의 타입에 따라 함수가 호출되고, C#의 경우 객체의 타입에 타입을 고려하여 함수를 호출한다.
